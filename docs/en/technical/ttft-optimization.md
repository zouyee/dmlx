...
用并发原语并行化可以将首次运行从 67s 降到 ~10s。

**使用 Zig 0.16 `std.Io.Group`**（项目 `minimum_zig_version = "0.16.0"`）:

`std.Io.Group` 是 Zig 0.16 新增的无序任务组原语，比 `std.Thread.Pool` 更适合此场景：
- macOS 上 `Io.Evented` 解析为 `Dispatch.zig`（GCD 后端），用 `dispatch_queue_t` 调度，
  内核级线程池，比用户态线程池调度开销更低
- API 更简洁：`group.async` + `group.await` 两行搞定
- 原生支持 `group.cancel`，对 Ctrl-C 中断友好
...
pub fn buildIndexFromDirectoryParallel(
    ...
    // 每个 shard 一个独立的 TensorIndex，避免共享 HashMap 竞争
    ...
    // Group.async: 任务数可以超过线程数，超出部分在调用线程同步执行（见下方分析）
    var group: Io.Group = .init;
    for (shard_paths, 0..) |path, i| {
        group.async(io, parseShardWorker, .{ &per_shard[i], allocator, path });
    }
    try group.await(io);  // 阻塞直到所有 shard 解析完成

    // 单线程合并所有 per-shard index
...
fn parseShardWorker(
    ...
    result.addShard(shard_path) catch |e| {
        std.log.err("Failed to parse shard {s}: {}", .{ shard_path, e });
    };
}
```

**`Group.async` 任务数超过线程数时的行为（源码确认）**:

issue [ziglang/zig#26027](https://github.com/ziglang/zig/issues/26027) 报告了
`Io.Group` 在任务数超过线程数时可能 hang。源码分析结论：**在 0.16.0 中不会 hang**。

`Threaded.groupAsync`（`std/Io/Threaded.zig:2176`）的关键逻辑：

```
1. 检查 busy_count >= async_limit
   → 是: 销毁 task，调用 groupAsyncEager(start, context)
          即在调用线程同步执行该任务，不入队
   → 否: busy_count += 1，入队 run_queue，唤醒 worker
```

当 33 个 shard 任务提交时（假设 10 核 Mac，async_limit = 9）：
- 前 9 个任务入队，分配给 worker 线程并行执行
- 第 10-33 个任务触发 `busy_count >= async_limit`，**在调用线程同步执行**（`groupAsyncEager`）
- 同步执行的任务不入队、不占 busy_count，不会死锁

这是一个优雅降级机制：超出并发上限的任务退化为串行，不会 hang。
但这也意味着并行度受限于 `async_limit`（默认 = CPU 核数 - 1）。
10 核 Mac 上最多 9 个 shard 并行 + 24 个串行，实际加速比约 3-4x 而非理论 9x。

macOS `Dispatch.zig` 后端的 `groupAsync` 行为类似：失败时回退到同步执行。

**与 `std.Thread.Pool` 的对比**:

| | `std.Thread.Pool` | `Io.Group` (Threaded) | `Io.Group` (Dispatch/GCD) |
|---|---|---|---|
| 调度后端 | 用户态线程池 | 用户态线程池 | macOS 内核 GCD |
| 超限行为 | 排队等待（可能 hang） | 同步执行（不 hang） | fiber 调度（不 hang） |
| 取消支持 | 无 | `group.cancel` | `group.cancel` |
| API 复杂度 | init/spawn/waitAll/deinit | group.async/group.await | 同左 |

**收益**: 首次运行 67s → ~15-20s（受 async_limit 限制，实际并行度 < 核心数）。

**注意**: 当前代码全部用 C posix 调用做文件 I/O（`@cImport("fcntl.h")` → `posix.open` → `pread`），
`Io.Group` 只负责并发调度，不要求任务内部用 `std.Io.File`。`parseShardWorker` 内部
仍然调用现有的 `TensorIndex.addShard`（C posix I/O），完全兼容。

**注意**: 这不是独立方案，而是 P0 的补充 — 只在首次运行（无缓存）时触发。
...
5. **P0.5 并行 header 解析**（使用 Zig 0.16 `std.Io.Group`）
   - 仅在无缓存时触发
   - `Io.Group.async` 并行解析 shard header，macOS 走 GCD 后端
   - 每 shard 独立 TensorIndex，最后单线程合并
   - 预期收益: 首次运行 67s → ~15-20s（受 async_limit 限制）
