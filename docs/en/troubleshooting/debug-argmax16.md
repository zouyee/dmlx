...
**影响**: 每一层的 residual connection 都用了错误的 comb 矩阵，导致所有层输出偏差。
这会累积到最终 logits，可能是 argmax=16 的根本原因。

### P5.2b — mhcPost 还缺少 float32 转换
Python 在 post 中做了 `.astype(mx.float32)` 再计算，最后 `.astype(x.dtype)` 转回。
Zig 没有做 float32 转换。 → **已修复**
...
如果 takeAxis 切片破坏了 mxfp4 tensor 的内部格式（strides 不连续），
gather_qmm 可能产生错误结果。
