...
> 以及 `mlx-zig` 实际能力（含 `CustomMetalKernel` 自定义 Metal kernel 支持），
> 制定的可执行优化路线图。
>
...
本次审计发现 `mlx-zig` 在以下方面存在可优化的空间，按影响程度分为 P0-P3 四级。

---

## 二、关键架构差异（v2 新增）

在制定具体任务前，必须理解 `mlx-zig` 与 `mlx-lm` 在**核心模块设计**上的深层差异。这些差异不是简单的"实现缺失"，而是架构路径不同，需要针对性重构。
...
**结论**：mlx-zig 的 MoE 是一条 `fused → split → dequantize → per-expert loop` 路径，与 mlx-lm 的 `fused → gather_mm` 路径完全相反。优化需要**反向重构**。
...
**结论**：mlx-zig 缺少 `Compressor` 模块和 `DeepseekV4Cache` 状态机，不是简单修改 `compressKV` 函数即可。
...
**结论**：两个 Indexer 的权重形状、输入来源、打分逻辑**完全不同**。mlx-zig 的 `LightningIndexer` 更接近 DeepSeek V3 论文原始设计，而 mlx-lm 的是 Apple 的重新实现。需要明确选择对齐方向。
...
| **mlx-zig 行动** | 1. ~~绑定 `mlx_gather_mm`~~（已完成）<br>2. **停止 splitFusedExperts**：loader 中保留 `switch_mlp.weight` 为 `[n_experts, out, in]` fused 格式<br>3. **重构 DSV4MoE**：删除 `DSV4Expert` 数组，改用 fused 权重 + `gatherMm`/`gatherQmm`<br>4. **接入 sort**：路由后按专家 ID `argsort` token，用 `sorted_indices=true` 调用 gather<br>5. **scores 前置融合**：在 down-proj 前乘入 |
...
| **mlx-zig 行动** | 1. **引入 `Compressor` 结构体**：封装 `wkv`、`wgate`、`ape`、`norm`、`overlap` 标志<br>2. **引入 `DeepseekV4Cache`**：扩展或新增 cache 类型，管理 `local` + `compressor_state` + `indexer_state`<br>3. **实现 `_overlap_transform`**：在 `Compressor` 中增加 overlap 分支（仅 `compress_ratio == 4`）<br>4. **实现 `accumulate_windows`**：在 `DeepseekV4Cache` 中维护 buffer 状态 |
...
| 属性 | 内容 |
|------|------|
| **问题** | ~~mlx-zig 的 loader 对所有 `compress_ratio > 1` 层创建 `LightningIndexer`~~ |
...
| 属性 | 内容 |
|------|------|
| **问题** | mlx-zig 当前对所有 CSA 层统一走 `gatherBlocks → concat → dense SDPA`，未区分 generate（L=1）和 prefill（L>1）。**这是正确性问题**：generate 阶段 L=1 时 compressed blocks 的 gather 语义不同（应用上次 prefill 的 index 结果） |
...
| **mlx-zig 行动** | 1. 在 `DSV4Attention.forward` 中增加 `if (seq_len == 1)` gather + dense SDPA 分支<br>2. 实现 `seq_len > 1` 的 `_sparse_pooled_attention` 等效路径 |
...
| 属性 | 内容 |
|------|------|
| **问题** | `LightningIndexer.quantize4bit()`（`deepseek_v4.zig:905-986`）做 `quantize(mxfp4) → dequantize → float32 matmul`，既无加速也无精度优势。但更重要的是：mlx-zig 的 `LightningIndexer` 与 mlx-lm 的 `Indexer` **架构完全不同**（见 §2.3） |
| **mlx-lm 参考** | `Indexer.__call__`（`deepseek_v4.py:1567-1598`）全程 bfloat16/float32，使用内部 `Compressor` + `wq_b` 投影 + `weights_proj` 加权 |
| **mlx-zig 行动** | **方案 A（推荐）**：对齐 mlx-lm 的 Indexer 架构<br>1. 删除 `LightningIndexer`，改为 `Indexer` 结构体（嵌套 `Compressor`，`wq_b` 投影，`weights_proj`）<br>2. 删除 `quantize4bit`<br><br>**方案 B**：保留论文原始 Lightning Indexer 设计，仅删除 `quantize4bit`，改用 bfloat16 matmul |
...
| **mlx-zig 行动** | 1. 将 mlx-lm 的 Metal source 字符串翻译为 Zig 字符串<br>2. 通过 `src/ops/custom_kernel.zig` 注册 `CustomMetalKernel`<br>3. 触发条件：`!training && hc_mult==4 && dtype==bfloat16 && gpu`<br>4. 模块加载时静态初始化 kernel（仿照 mlx-lm 的 `_hc_sinkhorn_collapse_kernel = _make_hc_sinkhorn_collapse_kernel()`）<br>5. Fallback 到现有 ops 路径 |
...
| **mlx-zig 行动** | 在 `Compressor` 中按 `compress_ratio` 值选择策略：128x 层简化计算（无 overlap，out_dim = head_dim） |
...
| 属性 | 内容 |
|------|------|
| **问题** | mlx-zig 的 `sink_logits` 实现与 mlx-lm dense SDPA 路径**已经对齐**（都传给 `fast_scaled_dot_product_attention` 的 `sinks` 参数）。真正的差异在 `_sparse_pooled_attention` 路径：mlx-lm 手动将 sink concat 到 scores 前端（`deepseek_v4.py:319-324`），mlx-zig 没有 sparse_pooled 路径 |
| **mlx-zig 行动** | 在实现 `_sparse_pooled_attention` 等效路径时，显式将 sink 作为前缀 concat 到 `scores`（`scores = concat([sink_scores, local_scores, pooled_scores])`） |
...
| 属性 | 内容 |
|------|------|
| **问题** | mlx-zig 中 `DSV4YarnRoPE` 可能有独立的 `apply` 和 `applyInverse` 方法，代码重复 |
| **mlx-lm 参考** | `_apply_partial_rope(..., inverse=True)`（`deepseek_v4.py:266-284`）复用同一函数 |
| **mlx-zig 行动** | 将 `applyInverse` 合并到 `apply` 中，增加 `inverse: bool = false` 参数 |
| **预期收益** | 减少代码重复，降低维护成本 |
...
| **mlx-zig 行动** | 1. 加载时保留 packed weight + scales（不 dequantize）<br>2. 专家矩阵乘改用 `gatherQmm`（`quantize.zig:324-358`）<br>3. 注意：这要求先完成 P0.1 的 fused 格式重构（不 split） |
...
| 属性 | 内容 |
|------|------|
| **问题** | mlx-zig 的 O-LoRA 手动实现 grouped matmul（`deepseek_v4.zig:1305-1358`），只有 float32/float16 路径，**无量化 matmul** |
...
| **mlx-zig 行动** | 在 grouped projection 中检测 `wo_a` 是否为量化权重，是则调用 `quantizedMatmul`（`quantize.zig:259-285`） |
| **预期收益** | 量化模型端到端推理性能提升 |
...
| 属性 | 内容 |
|------|------|
| **问题** | `mlx-c` 提供 `mlx_gather_mm`（`ops.h:467`：非量化 gather matmul，支持 `lhs_indices`/`rhs_indices`/`sorted_indices`），但 mlx-zig 未绑定。当前只有 `gatherQmm`（量化版） |
| **mlx-zig 行动** | 在 `ops.zig` 或新模块中绑定 `mlx_gather_mm`：<br>```zig<br>pub fn gatherMm(ctx, a, b, lhs_indices, rhs_indices, sorted_indices) !Array<br>``` |
...
| 属性 | 内容 |
|------|------|
| **问题** | mlx-lm `sanitize`（`deepseek_v4.py:1992-2127`）处理 FP4/FP8 自定义反量化（`dequant_fp4`、`dequant_fp8`）、命名 remap（`w1→gate_proj`）、以及 **独立 expert → fused `switch_mlp` 的 stack**。mlx-zig 的方向相反（fused → split） |
| **mlx-zig 行动** | 1. 确认 mlx-lm 量化格式（packed uint8 + scales 的特定布局）是否与 `mlx_dequantize` 兼容<br>2. 随着 P0.1 重构，loader 最终应**停止 split**，改为保留 fused 格式（与 mlx-lm 同向） |
...
| `custom_kernel.zig` | `mlx-zig/src/ops/custom_kernel.zig` | mlx-zig CustomMetalKernel（完整 API） |
| `deepseek_v4.zig` | `mlx-zig/src/models/deepseek_v4.zig` | mlx-zig V4 实现（2332 行） |
| `deepseek_v4_loader.zig` | `mlx-zig/src/models/deepseek_v4_loader.zig` | 权重加载（1169 行） |
| `moe_router.zig` | `mlx-zig/src/moe_router.zig` | MoE 路由模块（未接入 DSV4MoE） |
| `quantize.zig` | `mlx-zig/src/quantize.zig` | 量化 / gather_qmm / 无 gather_mm |
| `ops.h` | `mlx-c/mlx/c/ops.h` | `mlx_gather_mm` / `mlx_gather_qmm` C API |

---

## 十、附录：mlx-lm vs mlx-zig 能力矩阵（v2 修订）

| 特性 | mlx-lm (Python) | mlx-zig | 差距 | 备注 |
|------|-----------------|---------|------|------|
...
| P2 5.2 (原 P2 5.2) | 建议"sanitize 对齐 + stack" | 指出 mlx-zig 方向与 mlx-lm 相反；改为"停止 split，保留 fused" |
