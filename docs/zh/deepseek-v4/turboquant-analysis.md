# DeepSeek V4 + TurboQuant 论文分析：对 dmlx 的参考价值（更新版）

> 基于 DeepSeek V4 技术报告（2026-04-24）和 TurboQuant 论文（arXiv:2504.19874）的分析，
> 结合 dmlx 代码审计（2026-04-26）和刚完成的 MXFP4/FP8 绑定实现。

---

## 一、当前实现状态审计

在分析论文参考价值之前，先确认 dmlx 中 V4 相关功能的真实实现状态：

### 已完成（代码已存在且功能完整）

| 任务 | 实现位置 | 状态 |
|------|---------|------|
| 15.2 可学习 softmax-gated pooling | `compressKV()` → `softmaxGatedPool()` 位于 L1231-1430 | ✅ 完整实现，含 remainder 处理 |
| 15.4 FP4 Lightning Indexer | `LightningIndexer` 结构体位于 L648-930 | ✅ 完整实现，含 INT4 量化模拟、top-k 选择、block gather |
| 15.5 Attention Sink | `sink_logits` 字段位于 L959，传入 `scaledDotProductAttention` 位于 L1144 | ✅ 已接入 fast SDPA |
| 15.3 FP8 KV 存储 | `kv_storage_dtype` + `astype` 位于 L1055-1080 | ⚠️ 用 float16 代替 FP8（注释说明 MLX 缺原生 FP8） |
| 15.6 异构 KV cache | `compress_ratios` 从 config 读取，per-layer `compress_ratio` 位于 L950 | ⚠️ 压缩比已 per-layer，但 KV cache strategy 仍统一 |
