...
MLX-Zig 是基于 Apple MLX C 绑定（`mlx-c`）的全栈 LLM 推理与训练系统，以 Zig 语言编写，目标平台为 macOS Apple Silicon。项目经历了从原型到生产级的显著跃迁，当前具备与 Python vLLM/mlx-lm 对标的功能深度，包括 DeepSeek V4 完整支持、六级 KV Cache 策略、投机解码双轨制、引导解码 FSM、分层缓存（RAM+SSD）、以及多 Mac 分布式推理。
...
| 源码行数（不含测试） | ~42,455 行 |
| 测试模块数 | 50+ |
| 通过测试数 | 350（据 production-roadmap.md） |
...
**项目声称已修复**：`production-roadmap.md` 中"安全：`@constCast` 绕过 CoW → 全部改用 mlx-c 算子链 ✅"
...
`production-roadmap.md` 声称所有 Phase 0–7 + Task 13–34 已完成，350 测试通过。本分析验证：
...
| `production-roadmap.md` | 路线图 | Phase 0-7 进度追踪 |
...
**构建产物**：
- `libmlx-zig.a`：静态库
- `mlx-zig`：CLI 工具（chat/serve/benchmark/quantize/lora-train/convert/evaluate）
- `example`：示例程序
