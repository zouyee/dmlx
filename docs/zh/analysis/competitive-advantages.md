...
> 更新于 2026-04-26，包含端到端推理验证 (TinyLlama-1.1B-Chat-v1.0-4bit + Qwen2.5-0.5B-Instruct)。
...
- ✅ `quantizedMatmul` 融合 kernel 用于所有线性层（7 投影 + lm_head）
- ✅ 350 单元测试全部通过
...
Zig 的 comptime 可根据模型配置在编译时生成特化代码：
- 模型注册表在编译时构建 (`std.StaticStringMap`)
- 量化码本是编译时常量（TurboQuant `Codebook.b1/b2/b3/b4`）
- 类型安全的 dtype 映射 (`dtypeOf(comptime T: type)`)

Python 方案在运行时进行这些分发，产生额外开销。
