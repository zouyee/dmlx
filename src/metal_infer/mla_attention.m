...
// --- N-token batched MLA attention (Phase B-1) ---
// Encodes the full MLA attention chain for n_tok tokens of one chunk into a
// single external_cb (caller commits+waits once). Mirrors mla_attention_decode_bf16
// per-token math but batches the 5 projection GEMMs (wq_a/wq_b/wkv/wo_a/wo_b)
// and the 3 RMS-norms across tokens (weight shared, L2-resident). RoPE, KV
// cache write, SDPA, and rope_inv stay per-token (RoPE needs per-position
// cos/sin; SDPA has cross-token KV causal dependency). Bit-exact vs N
// independent mla_attention_decode_bf16 calls by construction (same kernels,
// same per-(tok,row) accumulation, same dispatch order within the CB).
//
// Inputs (slot-strided b2_* arena, one MTLBuffer per type, slot t at offset
// t*stride): x_in [n_tok,DIM] bf16 (= b2_norm_bf16), and b2_q_a/q_res/q/q_n/
// kv/kv_n/attn_scr/concat/wo_out. kv_cache_gpu_buf is the layer's persistent
// [MAX_SEQ_LEN,KV_LORA_RANK] f16 cache, written at rows start_pos..start_pos+n_tok-1.
// out_gpu [n_tok,DIM] f32 receives wo_b output (= b2_wo_out).
int mla_attention_decode_batch_bf16(MlaPipes *P, const AttnWeights *aw,
    ...
    AttnBufCache *abc = attn_buf_cache_get(d, aw);
    if (!abc || !abc->wq_a_pack || !abc->wq_b_pack || !abc->wkv_pack ||
        !abc->wo_a_pack_all || !abc->wo_b_pack) {
        // Batched path needs the cached packed weights; fall back to per-token.
        return -2;
    }
    id<MTLBuffer> bx   = (__bridge id<MTLBuffer>)x_in_gpu;
    id<MTLBuffer> bq_a = (__bridge id<MTLBuffer>)b2_q_a_v;
    id<MTLBuffer> bq_res = (__bridge id<MTLBuffer>)b2_q_res_v;
    id<MTLBuffer> bq   = (__bridge id<MTLBuffer>)b2_q_v;
    id<MTLBuffer> bq_n = (__bridge id<MTLBuffer>)b2_q_n_v;
    id<MTLBuffer> bkv  = (__bridge id<MTLBuffer>)b2_kv_v;
    id<MTLBuffer> bkv_n = (__bridge id<MTLBuffer>)b2_kv_n_v;
    id<MTLBuffer> battn = (__bridge id<MTLBuffer>)b2_attn_scr_v;
    id<MTLBuffer> bconcat = (__bridge id<MTLBuffer>)b2_concat_v;
    id<MTLBuffer> bout = (__bridge id<MTLBuffer>)b2_wo_out_v;
    id<MTLBuffer> bkvc = (__bridge id<MTLBuffer>)kv_cache_gpu_buf;

    const uint half = QK_ROPE_DIM / 2;
    // Precompute [n_tok, half] cos/sin (per-position YaRN) into temp GPU buffers.
    // CPU writes before the CB commits → visible to all rope encoders.
    id<MTLBuffer> bcos_all = [d newBufferWithLength:(size_t)n_tok * half * sizeof(float)
                                            options:MTLResourceStorageModeShared];
    id<MTLBuffer> bsin_all = [d newBufferWithLength:(size_t)n_tok * half * sizeof(float)
                                            options:MTLResourceStorageModeShared];
    float *cosp = (float *)[bcos_all contents], *sinp = (float *)[bsin_all contents];
    for (int t = 0; t < n_tok; t++) {
        yarn_cos_sin(start_pos + t, cosp + t * half, sinp + t * half);
    }
    id<MTLBuffer> bqn_w = abc->q_norm_buf, bkvn_w = abc->kv_norm_buf, bsink = abc->attn_sink_buf;

    const size_t QDIM = (size_t)N_HEADS * HEAD_DIM;            // bf16 elements per token for q/q_n/attn_scr
    const size_t Qstride_b = QDIM * sizeof(uint16_t);
    const size_t KVstride_b = (size_t)KV_LORA_RANK * sizeof(uint16_t);
    const size_t Cstride_b = (size_t)O_GROUPS * O_LORA_RANK * sizeof(float);
    const size_t Ostride_b = (size_t)DIM * sizeof(float);
    const size_t cos_t_stride_b = (size_t)half * sizeof(float);

    // --- PASS 1: batched wq_a + wkv (both read bx) ---
    {
        id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
        [e setComputePipelineState:P->dequant_matvec_affine_bf16in_bf16out_batch];
        [e setBuffer:abc->wq_a_pack offset:0 atIndex:0]; [e setBuffer:abc->wq_a_sc offset:0 atIndex:1];
        [e setBuffer:abc->wq_a_bi offset:0 atIndex:2];   [e setBuffer:bx offset:0 atIndex:3]; [e setBuffer:bq_a offset:0 atIndex:4];
        uint od=Q_LORA_RANK, id_=DIM, gs=64, n=(uint)n_tok;
        ...
        [e dispatchThreads:MTLSizeMake(n_tok*Q_LORA_RANK,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
        [e endEncoding];
    }
    {
        id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
        [e setComputePipelineState:P->dequant_matvec_affine_bf16in_bf16out_batch];
        [e setBuffer:abc->wkv_pack offset:0 atIndex:0]; [e setBuffer:abc->wkv_sc offset:0 atIndex:1];
        [e setBuffer:abc->wkv_bi offset:0 atIndex:2];   [e setBuffer:bx offset:0 atIndex:3]; [e setBuffer:bkv offset:0 atIndex:4];
        uint od=KV_LORA_RANK, id_=DIM, gs=64, n=(uint)n_tok;
        [e setBytes:&od length:4 atIndex:5]; [e setBytes:&id_ length:4 atIndex:6]; [e setBytes:&gs length:4 atIndex:7]; [e setBytes:&n length:4 atIndex:8];
        [e dispatchThreads:MTLSizeMake(n_tok*KV_LORA_RANK,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
        [e endEncoding];
    }
    // --- PASS 2: batched q_norm (N rows, weight broadcast) + kv_norm ---
    enc_rms_norm_rows_bf16in_bf16out(P, cb, bq_a, bqn_w, bq_res, (uint)n_tok, Q_LORA_RANK, 1);
    enc_rms_norm_rows_bf16in_bf16out(P, cb, bkv, bkvn_w, bkv_n, (uint)n_tok, KV_LORA_RANK, 1);
    // --- PASS 3: batched wq_b ---
    {
        id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
        [e setComputePipelineState:P->dequant_matvec_affine_bf16in_bf16out_batch];
        [e setBuffer:abc->wq_b_pack offset:0 atIndex:0]; [e setBuffer:abc->wq_b_sc offset:0 atIndex:1];
        [e setBuffer:abc->wq_b_bi offset:0 atIndex:2];   [e setBuffer:bq_res offset:0 atIndex:3]; [e setBuffer:bq offset:0 atIndex:4];
        uint od=(uint)QDIM, id_=Q_LORA_RANK, gs=64, n=(uint)n_tok;
        [e setBytes:&od length:4 atIndex:5]; [e setBytes:&id_ length:4 atIndex:6]; [e setBytes:&gs length:4 atIndex:7]; [e setBytes:&n length:4 atIndex:8];
        [e dispatchThreads:MTLSizeMake(n_tok*QDIM,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
        [e endEncoding];
    }
    // --- PASS 4: batched head-norm (n_rows = n_tok*N_HEADS, row_dim=HEAD_DIM, no weight) ---
    enc_rms_norm_rows_bf16in_bf16out(P, cb, bq, NULL, bq_n, (uint)n_tok * N_HEADS, HEAD_DIM, 0);

    // --- PASS 5a: per-token RoPE(Q) + RoPE(KV) + KV cache write (cb1) ---
    // All KV rows for the chunk are written here; cb1 is committed+waited
    // before SDPA so token t+1's SDPA (cb2) is guaranteed to see token t's KV
    // write. (Intra-CB dispatch-order visibility was insufficient for the
    // cross-token KV case — bit-exact smoke failed until this split.)
    for (int t = 0; t < n_tok; t++) {
        NSUInteger qoff = (NSUInteger)t * Qstride_b;
        NSUInteger kvoff = (NSUInteger)t * KVstride_b;
        NSUInteger coff = (NSUInteger)t * cos_t_stride_b;
        int pos = start_pos + t;
        // RoPE forward on Q (n_heads=N_HEADS)
        {
            id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
            [e setComputePipelineState:P->rope_tail_interleaved_bf16];
            [e setBuffer:bq_n offset:qoff atIndex:0];
            [e setBuffer:bcos_all offset:coff atIndex:1]; [e setBuffer:bsin_all offset:coff atIndex:2];
            uint nh=N_HEADS, hd=HEAD_DIM, nd=QK_NOPE_DIM, rd=QK_ROPE_DIM, inv=0;
            [e setBytes:&nh length:4 atIndex:3]; [e setBytes:&hd length:4 atIndex:4];
            [e setBytes:&nd length:4 atIndex:5]; [e setBytes:&rd length:4 atIndex:6]; [e setBytes:&inv length:4 atIndex:7];
            [e dispatchThreads:MTLSizeMake(N_HEADS*half,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
            [e endEncoding];
        }
        // RoPE forward on KV (n_heads=1)
        {
            id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
            [e setComputePipelineState:P->rope_tail_interleaved_bf16];
            [e setBuffer:bkv_n offset:kvoff atIndex:0];
            [e setBuffer:bcos_all offset:coff atIndex:1]; [e setBuffer:bsin_all offset:coff atIndex:2];
            uint nh=1, hd=HEAD_DIM, nd=QK_NOPE_DIM, rd=QK_ROPE_DIM, inv=0;
            [e setBytes:&nh length:4 atIndex:3]; [e setBytes:&hd length:4 atIndex:4];
            [e setBytes:&nd length:4 atIndex:5]; [e setBytes:&rd length:4 atIndex:6]; [e setBytes:&inv length:4 atIndex:7];
            [e dispatchThreads:MTLSizeMake(half,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
            [e endEncoding];
        }
        // KV cache write: bkv_n[t] → kvcache[start_pos+t] (bf16→f16)
        {
            id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
            [e setComputePipelineState:P->bf16_to_f16_row];
            [e setBuffer:bkv_n offset:kvoff atIndex:0];
            [e setBuffer:bkvc offset:0 atIndex:1];
            uint row = (uint)pos, rank = KV_LORA_RANK;
            [e setBytes:&row length:4 atIndex:2]; [e setBytes:&rank length:4 atIndex:3];
            [e dispatchThreads:MTLSizeMake(KV_LORA_RANK,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
            [e endEncoding];
        }
    }
    // KV visible: cb1 wrote all chunk KV rows. Pass 5b (SDPA) reads them.
    // Intra-CB dispatch-order visibility (same as per-token merged_cb) makes
    // cross-token KV reads safe without a mid-CB commit — verified by smoke
    // + bit-exact multi-chunk (the original gibberish was the wo_b naive-kernel
    // bug, not a KV hazard).
    // --- PASS 5b: per-token SDPA + rope_inv ---
    for (int t = 0; t < n_tok; t++) {
        NSUInteger qoff = (NSUInteger)t * Qstride_b;
        NSUInteger coff = (NSUInteger)t * cos_t_stride_b;
        int pos = start_pos + t;
        // SDPA: bq_n[t], KV[0..pos+1] → battn[t]
        {
            id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
            [e setComputePipelineState:P->mla_sdpa_decode_bfloat];
            [e setBuffer:bq_n offset:qoff atIndex:0];
            [e setBuffer:bkvc offset:0 atIndex:1];
            [e setBuffer:bsink offset:0 atIndex:2];
            [e setBuffer:battn offset:qoff atIndex:3];
            uint nh=N_HEADS, hd=HEAD_DIM, nk=(uint)(pos + 1);
            float scale=1.0f/sqrtf((float)HEAD_DIM);
            [e setBytes:&nh length:4 atIndex:4]; [e setBytes:&hd length:4 atIndex:5];
            [e setBytes:&nk length:4 atIndex:6]; [e setBytes:&scale length:4 atIndex:7];
            [e dispatchThreadgroups:MTLSizeMake(N_HEADS,1,1) threadsPerThreadgroup:MTLSizeMake(32,1,1)];
            [e endEncoding];
        }
        // rope_inv on battn[t]
        {
            id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
            [e setComputePipelineState:P->rope_tail_interleaved_bf16];
            [e setBuffer:battn offset:qoff atIndex:0];
            [e setBuffer:bcos_all offset:coff atIndex:1]; [e setBuffer:bsin_all offset:coff atIndex:2];
            uint nh=N_HEADS, hd=HEAD_DIM, nd=QK_NOPE_DIM, rd=QK_ROPE_DIM, inv=1;
            [e setBytes:&nh length:4 atIndex:3]; [e setBytes:&hd length:4 atIndex:4];
            [e setBytes:&nd length:4 atIndex:5]; [e setBytes:&rd length:4 atIndex:6]; [e setBytes:&inv length:4 atIndex:7];
            [e dispatchThreads:MTLSizeMake(N_HEADS*half,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
            [e endEncoding];
    ...
    // --- PASS 6: batched wo_a (grp8 × n_tok) → bconcat, then wo_b → bout ---
    {
        id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
        [e setComputePipelineState:P->dequant_matvec_affine_bf16in_f32out_grp8_batch];
        [e setBuffer:abc->wo_a_pack_all offset:0 atIndex:0];
        [e setBuffer:abc->wo_a_sc_all offset:0 atIndex:1];
        [e setBuffer:abc->wo_a_bi_all offset:0 atIndex:2];
        [e setBuffer:battn offset:0 atIndex:3];   // [n_tok, 8, in_dim] grouped
        [e setBuffer:bconcat offset:0 atIndex:4]; // [n_tok, 8*O_LORA_RANK] f32
        uint og=O_LORA_RANK, id_=DIM, gs=64, n=(uint)n_tok;
        [e setBytes:&og length:4 atIndex:5]; [e setBytes:&id_ length:4 atIndex:6];
        [e setBytes:&gs length:4 atIndex:7]; [e setBytes:&n length:4 atIndex:8];
        [e dispatchThreads:MTLSizeMake(n_tok*O_GROUPS*O_LORA_RANK,1,1) threadsPerThreadgroup:MTLSizeMake(256,1,1)];
        [e endEncoding];
    }
    // wo_b: per-token via the SAME v2 kernel the per-token path uses (engine.c:986).
    // Bit-exact with baseline. A perf probe (NATIVE_BATCH2=1 + naive batched wo_b)
    // measured 100.46s vs 99.50s for per-token v2 — batching wo_b REGRESSES (L2
    // already amortizes its 20MB weight in the per-token path, and v2's tiled SIMD
    // is faster than the naive 1-thread/row batched kernel). So wo_b stays per-token.
    for (int t = 0; t < n_tok; t++) {
        id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
        [e setComputePipelineState:P->dequant_matvec_affine_v2];
        [e setBuffer:abc->wo_b_pack offset:0 atIndex:0];
        [e setBuffer:abc->wo_b_sc   offset:0 atIndex:1];
        [e setBuffer:abc->wo_b_bi   offset:0 atIndex:2];
        [e setBuffer:bconcat offset:(NSUInteger)t * Cstride_b atIndex:3];
        [e setBuffer:bout    offset:(NSUInteger)t * Ostride_b atIndex:4];
        uint od=DIM, id_=O_GROUPS*O_LORA_RANK, gs=64;
        ...
        uint num_tgs = (DIM + 1) / 2;
        [e dispatchThreadgroups:MTLSizeMake(num_tgs,1,1) threadsPerThreadgroup:MTLSizeMake(32,4,1)];
        [e endEncoding];
    }
    [cb commit]; [cb waitUntilCompleted];
    } // end @autoreleasepool
