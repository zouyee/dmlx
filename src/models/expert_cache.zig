...
pub const DEFAULT_MAX_BYTES: usize = 4 * 1024 * 1024 * 1024;
...
/// A single cache entry, threaded into the LRU doubly-linked list.
pub const CacheEntry = struct {
    key: CacheKey,
    tensor: Array,
    byte_size: usize,
    /// Previous entry in LRU list (toward head / MRU)
    lru_prev: ?*CacheEntry,
    /// Next entry in LRU list (toward tail / LRU)
    lru_next: ?*CacheEntry,
};
...
/// LRU cache for expert weight tensors.
///
/// Head = most recently used, tail = least recently used.
/// On `get()` hit, the entry is promoted to head.
/// On `put()`, LRU tail entries are evicted until the budget allows the new entry.
pub const ExpertCache = struct {
    ...
    ///
    /// Returns the tensor on hit (and promotes the entry to LRU head),
    ...
    /// Otherwise, LRU entries are evicted until there is room, then the new
    /// entry is inserted at the LRU head.
    ...
    /// Evict least-recently-used entries until `current_bytes + needed_bytes <= max_bytes`.
    fn evictUntil(self: *ExpertCache, needed_bytes: usize) void {
        while (self.current_bytes + needed_bytes > self.max_bytes) {
            ...
            std.log.debug("ExpertCache: evicting layer={d} expert={d}", .{ tail.key.layer_idx, tail.key.expert_id });
...
test "ExpertCache: LRU eviction order" {
    const allocator = std.testing.allocator;
    ...
    cache.put(key_a, t_a, 100); // [A]
    cache.put(key_b, t_b, 100); // [B, A]

    // Cache is full (200/200). Inserting C should evict A (LRU tail).
    cache.put(key_c, t_c, 100); // [C, B] — A evicted
...
test "ExpertCache: get promotes entry, changes eviction order" {
    const allocator = std.testing.allocator;
    var cache = ExpertCache.init(allocator, 200);
    defer cache.deinit();
    ...
    // Access A to promote it to head: [A, B]
    _ = cache.get(key_a);

    // Now inserting C should evict B (the new LRU tail), not A
    cache.put(key_c, t_c, 100); // [C, A] — B evicted

    try std.testing.expect(cache.get(key_b) == null); // B evicted
    try std.testing.expect(cache.get(key_a) != null); // A still present
    try std.testing.expect(cache.get(key_c) != null); // C present
}
...
test "ExpertCache: default max bytes constant" {
    try std.testing.expectEqual(@as(usize, 4 * 1024 * 1024 * 1024), DEFAULT_MAX_BYTES);
}
...
test "Feature: stream-mode-performance, Property 2: LRU eviction order" {
    ...
    // whose most recent access is oldest among all entries.
    const allocator = std.testing.allocator;
    ...
    while (iteration < 100) : (iteration += 1) {
        ...
        // Randomly access one of the first two entries to change LRU order
        const access_idx = random.intRangeAtMost(usize, 0, 1);
        _ = cache.get(keys[access_idx]);

        // The LRU entry is the one NOT accessed (among keys[0] and keys[1])
        // that was inserted earliest. keys[2] was most recently inserted.
        // After accessing keys[access_idx], the LRU tail should be the other
        // of keys[0]/keys[1] that was NOT accessed.
        const expected_evicted = if (access_idx == 0) @as(usize, 1) else @as(usize, 0);

        // Insert a 4th entry — should evict the LRU entry
        keys[3] = CacheKey{
            .layer_idx = 99,
            .tensor_name_hash = random.int(u64),
            .expert_id = 0,
        ...
        // The expected_evicted key should be gone
        try std.testing.expect(cache.get(keys[expected_evicted]) == null);
        // The accessed key should still be present
        try std.testing.expect(cache.get(keys[access_idx]) != null);
        // keys[2] should still be present
        try std.testing.expect(cache.get(keys[2]) != null);
